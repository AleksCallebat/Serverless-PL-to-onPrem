# Ensure the storage account exists before referencing it
# Add a check or create the storage account if it doesn't exist
...
# Update the data block to depend on the storage account creation
data "azurerm_storage_account" "dbfs" {
  name                     = azurerm_storage_account.dbfs.name
  resource_group_name      = azurerm_storage_account.dbfs.resource_group_name
  depends_on               = [azurerm_storage_account.dbfs]
}
